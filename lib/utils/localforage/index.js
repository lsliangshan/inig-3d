...
export const jobStore = localforage.createInstance({
  name: 'zpm',
  storeName: 'job'
})
...
export const baseDataVersionStore = localforage.createInstance({
  name: 'zpm',
  storeName: 'base-data-version'
})

export const baseDataStore = localforage.createInstance({
  name: 'zpm',
  storeName: 'base-data'
})

export const workerStore = localforage.createInstance({
  name: 'zpm',
  storeName: 'worker'
})
  ...
  return new Promise(async (resolve) => {
    if (!args.store) {
      ...
      await args.store.keys().then(keys => {
        resolve(keys.filter(item => item.indexOf(args.key) > -1))
      }).catch(err => {
        resolve([])
      })
    }
...
export function getItem (args) {
  return new Promise(async (resolve, reject) => {
    if ((process.env.inig_isServer === 'true') || (process.env.inig_isTest === 'true')) {
      resolve(null)
    } else if (!args.store) {
