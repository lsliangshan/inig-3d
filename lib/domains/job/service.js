...
export function requestAreaJobs (args) {
  return new Promise(async (resolve, reject) => {
    ...
    if (!args || !args.companyNumber) {
      ...
      await Job.requestAreaJobs({
        S_SOU_POSITION_TYPE: '1;2;4',
        facets: 'SOU_WORK_CITY',
        S_SOU_COMPANY_ID: formatArgs.companyNumber,
        S_SOU_WORK_CITY: formatArgs.workCity || '',
        pageIndex: formatArgs.pageIndex || 1,
        pageSize: formatArgs.pageSize || 10
      }).then(res => {
        resolve(res)
      }).catch(err => {
        reject(new Error(err.message || '搜索失败，请稍后再试'))
...
export function requestJobDeliver (args) {
  return new Promise(async (resolve, reject) => {
    ...
    if (!args || !args.number || (args.number.length < 1)) {
      reject(new Error('职位编号不能为空'))
    } else {
